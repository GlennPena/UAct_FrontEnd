  ...
  const updateSubmissionStatus = async (submissionId, newStatus) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        // 1. Update submissions state for the modal
        ...
        // 2. CONDITIONALLY UPDATE PARENT PROGRAMS STATE (NEW LOGIC)

        if (newStatus === 'approved' && selectedProgram) {

           

            // Find the submission being updated to ensure it was pending

            const submissionToUpdate = submissions.find(s => s.id === submissionId);

           

            // Only update the slot count if the status was changing from PENDING to APPROVED

            // and the previous status wasn't already approved (to prevent double counting)

            if (submissionToUpdate && submissionToUpdate.status.toUpperCase() === 'PENDING') {

               

                // Update the main list of programs

                setPrograms(prevPrograms => prevPrograms.map(p => {

                    // Check if this is the program associated with the submission
                    ...
                    if (p.id === selectedProgram.id) {
                        ...
                        return {
                            ...
                            slots_taken: (p.slots_taken || 0) + 1 // Increment taken slots

                            // You might also need to update slots_remaining if it's explicitly stored
    ...
    <View style={[styles.container, { flex: 1, paddingTop: 40, alignItems: 'center' }]}>
      ...
      <View style={{
        ...
        <View style={{
          ...
          <Text style={{ fontSize: 26, fontWeight: 'bold', color: 'white' }}>Review Submissions</Text>
